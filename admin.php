 ...
 * @name plugins_faqmulti_admin
 */
class plugins_faqmulti_admin extends plugins_faqmulti_db
{
    /**
     * @var object
     */
    ...
    /**
     * plugins_faqmulti_admin constructor.
     */
	public function __construct()
    {
        ...
        if($this->controller === 'faqmulti') $this->controller = 'home';
        ...
        if (http_request::isPost('content')) {
            ...
            foreach($array as $key => $arr) {
                foreach($arr as $k => $v) {
                    $array[$key][$k] = ($k == 'desc_faqmulti') ? $formClean->cleanQuote($v) : $formClean->simpleClean($v);
                }
            }
            $this->content = $array;
        }
		...
		// --- Order
		if (http_request::isPost('faqmulti')) $this->faqmulti = $formClean->arrayClean($_POST['faqmulti']);
	}
	...
	{
		return $this->template->getConfigVars('faqmulti_plugin');
	}
    ...
    protected function order($type){
        switch ($type) {
            case 'home':
                for ($i = 0; $i < count($this->faqmulti); $i++) {
                    $this->upd(['type' => 'order', 'data' => ['id_faqmulti' => $this->faqmulti[$i], 'order_faqmulti' => $i]]);
                }
                break;
        }
    }
	 ...
	 * @return array
	 */
	protected function setItemfaqmultiData(array $data): array
	{
		$arr = [];
		if(!empty($data)) {

            foreach ($data as $faqmulti) {
                if (!array_key_exists($faqmulti['id_adv'], $arr)) {
                    $arr[$faqmulti['id_faqmulti']] = array();
                    $arr[$faqmulti['id_faqmulti']]['id_faqmulti'] = $faqmulti['id_faqmulti'];
                }
                $arr[$faqmulti['id_faqmulti']]['content'][$faqmulti['id_lang']] = [
                    'id_lang' => $faqmulti['id_lang'],
                    'title_faqmulti' => $faqmulti['title_faqmulti'],
                    'desc_faqmulti' => $faqmulti['desc_faqmulti'],
                    'published_faqmulti' => $faqmulti['published_faqmulti']
                ];
            }
        }
		return $arr;
